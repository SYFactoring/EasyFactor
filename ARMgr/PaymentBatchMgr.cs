    ...
    public partial class PaymentBatchMgr : UserControl
    {
        ...
        public PaymentBatchMgr(OpBatchType batchType)
        {
            ...
            if (batchType == OpBatchType.CHECK)
            {
                ...
                var queryResult = context.InvoicePaymentBatches.Where(i => i.CheckStatus == ConstStr.BATCH.UNCHECK);
        ...
        private void Check(object sender, EventArgs e)
        {
            ...
            if (batch.CheckStatus != ConstStr.BATCH.UNCHECK && !PermUtil.ValidatePermission(Permission.INVOICE_APPROVE))
            {
        ...
        private void DeleteBatch(object sender, EventArgs e)
        {
            ...
            if (MessageBoxEx.Show("是否打算删除此" + selectedBatch.BatchCount + "条付款记录", ConstStr.MESSAGE.TITLE_INFORMATION, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                return;
            }
